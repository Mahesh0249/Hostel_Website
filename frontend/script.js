...
const distanceHostelSelect = document.getElementById("distanceHostelSelect");
...
function setupDistanceOverlayToggle() {
  ...
  distanceTogglePanel.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();

    const collapsed = distanceOverlay.classList.toggle("is-collapsed");
    distanceTogglePanel.textContent = collapsed ? "+" : "−";
    distanceTogglePanel.setAttribute("aria-label", collapsed ? "Expand panel" : "Minimize panel");
    distanceTogglePanel.setAttribute("title", collapsed ? "Expand" : "Minimize");
    distanceTogglePanel.setAttribute("aria-expanded", String(!collapsed));
  });
}
...
function enableDistanceOverlayDrag() {
  ...
  function onPointerMove(event) {
    ...
    const maxLeft = distanceMapShell.clientWidth - distanceOverlay.offsetWidth;
    const maxTop = distanceMapShell.clientHeight - distanceOverlay.offsetHeight;

    const nextLeft = Math.max(0, Math.min(maxLeft, startLeft + dx));
    const nextTop = Math.max(0, Math.min(maxTop, startTop + dy));

    distanceOverlay.style.left = `${nextLeft}px`;
    distanceOverlay.style.top = `${nextTop}px`;
  }
  ...
  function stopDragging() {
    ...
    dragging = false;
    distanceOverlay.classList.remove("is-dragging");
    window.removeEventListener("pointermove", onPointerMove);
    window.removeEventListener("pointerup", stopDragging);
  }

  distanceDragHandle.addEventListener("pointerdown", (event) => {
    if (event.target.closest("#distanceTogglePanel")) {
      return;
    }
    ...
    startLeft = distanceOverlay.offsetLeft;
    startTop = distanceOverlay.offsetTop;
    distanceOverlay.classList.add("is-dragging");
...
function renderDistanceResult(location) {
  ...
  const selectedHostelName = String(hostelsData[selectedHostelKey]?.name || "Selected hostel");
  ...
  const payload = {
    ...
    hostelKey: selectedHostelKey
  };

  const apiHostel = apiHostelsByKey[selectedHostelKey];
  if (apiHostel?._id) {
    payload.hostelId = String(apiHostel._id);
  }
  ...
  apiFetchWithRetry(
    ...
    .catch((error) => {
      fallbackDistanceEstimate(location, selectedHostelKey, selectedHostelName, error);
    });
}
...
async function geocodeLocationFallback(locationText) {
  ...
  return {
    ...
    formattedAddress: String(results[0].display_name || locationText).trim()
  };
}

async function fallbackDistanceEstimate(location, selectedHostelKey, selectedHostelName, originalError) {
  if (!distanceResult) {
    return;
  }
  ...
  try {
    ...
    distanceResult.innerHTML = `<p>${escapeHtml(originalError?.message || "Could not calculate distance right now.")}</p>`;
    if (mapLink) {
      mapLink.hidden = true;
    }
  }
...
distanceHostelSelect?.addEventListener("change", () => {
  updateMapForSelectedHostel();
  const location = String(locationInput?.value || "").trim();
  if (location) {
    renderDistanceResult(location);
  }
});
...
const newAdminOwnership = document.getElementById("newAdminOwnership");
const newAdminAccessType = document.getElementById("newAdminAccessType");
...
function setAdminLoginCardVisibility(isVisible) {
  if (!adminLoginCard) {
    return;
  }

  if (isVisible) {
    adminLoginCard.removeAttribute("hidden");
    return;
  }

  adminLoginCard.setAttribute("hidden", "hidden");
}

function unlockAdminSections() {
  setAdminLoginCardVisibility(false);
...
async function tryAutoUnlockAdmin() {
  ...
  restoreAdminToken();
  if (!adminToken) {
    setAdminLoginCardVisibility(true);
    return;
  }
...
function fillHostelForm(hostelKey) {
  ...
  if (adminAccessType) {
    adminAccessType.value = data.accessType || "Boys";
  }
...
function renderAdmins(admins) {
  ...
  adminUsersCache = Array.isArray(admins) ? admins : [];
  ...
  adminsList.innerHTML = admins
    .map(
      ...
      <article class="admin-user-card">
        ...
        <p class="admin-user-meta"><strong>Ownership:</strong> ${escapeHtml(admin.ownership || "Managed")}</p>
        <p class="admin-user-meta"><strong>Access Type:</strong> ${escapeHtml(admin.access_type || "Limited Access")}</p>
        <p class="admin-user-meta"><strong>Created:</strong> ${escapeHtml(formatEnquiryTime(admin.createdAt))}</p>
        <div class="action-row">
          <button type="button" class="btn btn-outline admin-edit-btn" data-admin-id="${escapeHtml(String(admin._id || admin.id || ""))}">Edit</button>
          <button type="button" class="btn btn-outline admin-delete-btn" data-admin-id="${escapeHtml(String(admin._id || admin.id || ""))}">Remove</button>
          <button type="button" class="btn btn-outline admin-transfer-btn" data-admin-id="${escapeHtml(String(admin._id || admin.id || ""))}" ${String(admin.ownership || "") === "Owner" ? "disabled" : ""}>Transfer Ownership</button>
        </div>
      </article>
    `
    )
...
function setAdminFormMode(mode, admin = null) {
  ...
  if (editAdminId) {
    editAdminId.value = isEditMode ? String(admin?._id || admin?.id || "") : "";
  }

  if (createAdminSubmitBtn) {
    createAdminSubmitBtn.textContent = isEditMode ? "Save Admin Changes" : "Add Admin";
  }

  if (cancelAdminEditBtn) {
    if (isEditMode) {
      cancelAdminEditBtn.removeAttribute("hidden");
    } else {
      cancelAdminEditBtn.setAttribute("hidden", "hidden");
    }
  }

  if (newAdminPassword) {
    newAdminPassword.required = !isEditMode;
    newAdminPassword.placeholder = isEditMode ? "Leave blank to keep current password" : "";
  }

  if (newAdminPasswordConfirm) {
    newAdminPasswordConfirm.required = !isEditMode;
    newAdminPasswordConfirm.placeholder = isEditMode ? "Leave blank to keep current password" : "";
  }
  ...
  if (newAdminName) {
    newAdminName.value = String(admin?.name || "");
  }
  if (newAdminEmail) {
    newAdminEmail.value = String(admin?.email || "");
  }
  if (newAdminOwnership) {
    newAdminOwnership.value = String(admin?.ownership || "Managed");
  }
  if (newAdminAccessType) {
    newAdminAccessType.value = String(admin?.access_type || "Limited Access");
  }
  if (newAdminPassword) {
...
cancelAdminEditBtn?.addEventListener("click", () => {
  ...
  if (adminUserStatus) {
    adminUserStatus.textContent = "Edit cancelled.";
  }
});
...
adminsList?.addEventListener("click", async (event) => {
  ...
  if (editBtn) {
    const targetId = String(editBtn.getAttribute("data-admin-id") || "").trim();
    const selected = adminUsersCache.find((admin) => String(admin._id || admin.id) === targetId);
    if (!selected) {
      return;
    }
    setAdminFormMode("edit", selected);
    if (adminUserStatus) {
      adminUserStatus.textContent = "Editing selected admin. Update fields and save.";
    }
    return;
  }
  ...
  if (deleteBtn) {
    const targetId = String(deleteBtn.getAttribute("data-admin-id") || "").trim();
    if (!targetId) {
      return;
    }
    ...
    try {
      if (adminUserStatus) {
        adminUserStatus.textContent = "Removing admin...";
      }

      await apiFetchWithRetry(`/api/admin/${targetId}`, {
        method: "DELETE",
        headers: {
          ...getAuthHeaders()
        }
      ...
      if (adminUserStatus) {
        adminUserStatus.textContent = "Admin removed successfully.";
      }

      await hydrateAdminsFromApi();
    } catch (error) {
      if (adminUserStatus) {
        adminUserStatus.textContent = error.message || "Failed to remove admin.";
      }
    }
    return;
  }

  const transferBtn = event.target.closest(".admin-transfer-btn");
  if (transferBtn) {
    const targetId = String(transferBtn.getAttribute("data-admin-id") || "").trim();
    if (!targetId) {
      return;
    }

    if (!window.confirm("Transfer ownership to this admin? Current owner will become Managed.")) {
      return;
    }

    try {
      if (adminUserStatus) {
        adminUserStatus.textContent = "Transferring ownership...";
      }

      await apiFetchWithRetry(`/api/admin/${targetId}/transfer-ownership`, {
        method: "POST",
        headers: {
          ...getAuthHeaders()
        }
      ...
      if (adminUserStatus) {
        adminUserStatus.textContent = "Ownership transferred successfully.";
      }

      await hydrateAdminsFromApi();
    } catch (error) {
      if (adminUserStatus) {
        adminUserStatus.textContent = error.message || "Failed to transfer ownership.";
      }
    }
  }
});
...
createAdminForm?.addEventListener("submit", async (event) => {
  ...
  const targetAdminId = String(editAdminId?.value || "").trim();
  const isEditMode = Boolean(targetAdminId);
  ...
  const ownership = String(newAdminOwnership?.value || "Managed").trim();
  const accessType = String(newAdminAccessType?.value || "Limited Access").trim();
  ...
  try {
    if (adminUserStatus) {
      adminUserStatus.textContent = isEditMode ? "Updating admin user..." : "Creating admin user...";
    }
    ...
    await apiFetchWithRetry(isEditMode ? `/api/admin/${targetAdminId}` : "/api/admin", {
    ...
    if (adminUserStatus) {
      adminUserStatus.textContent = isEditMode ? "Admin updated successfully." : "Admin user created successfully.";
    }
...
adminLoginForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (adminLoginStatus) {
      adminLoginStatus.textContent = "Login successful. Redirecting to admin panel...";
    }
